...
# "memory://" keeps counters in-process, which means N Uvicorn workers allow
# N x the configured rate and every deploy resets the counters. Point
# RATE_LIMIT_STORAGE_URI at Redis ("redis://cache:6379") for limits that hold
# across workers and replicas.
RATE_LIMIT_STORAGE_URI = os.getenv("RATE_LIMIT_STORAGE_URI", "memory://").strip() or "memory://"

# "moving-window" counts hits over a rolling interval. The "fixed-window"
# alternative resets on wall-clock boundaries, which lets a client spend its
# full allowance at the end of one window and again at the start of the next —
# a 2x burst that is invisible in the limit string.
...
# Storage outages should not take the service down. With swallow_errors the
# limiter allows the request and logs instead of raising, i.e. it fails open:
# availability over enforcement. Flip RATE_LIMIT_FAIL_OPEN=false to fail closed
# where enforcement matters more than uptime.
...
limiter = Limiter(
    ...
    # Emits X-RateLimit-* and, on rejection, Retry-After. Off by default in
    # slowapi, so a 429 would otherwise tell the client nothing about when to
    # come back.
...
# Deliberately generous: this is a denial-of-service guard, not a quota. It has
# to sit above what a legitimate client — or several behind one office NAT —
# would ever produce, because the per-user limits below are what actually
# shape fair use.
...
# Paths that skip the throttle. Health checks are polled by orchestrators from
# a single address and would otherwise consume that address's whole budget;
# the docs endpoints are static and free to serve.
...
def _retry_after(item: RateLimitItem, key: str) -> int:
    ...
    try:
        ...
        # Never let a stats lookup turn a 429 into a 500 — the header is
        # advisory, the rejection is not.
...
async def throttle_by_ip(request: Request, call_next):
    ...
    if request.method == "OPTIONS" or request.url.path in PREAUTH_EXEMPT_PATHS:
        # CORS preflight carries no credentials and no body. Throttling it
        # would surface as an opaque CORS failure in the browser rather than a
        # readable 429.
    ...
    try:
        # Test every window before consuming any, so that a request rejected by
        # the daily limit does not also burn a slot in the per-minute one.
        rejected = next(
...
# 20 uploads per minute per user. slowapi limit-string syntax; multiple windows
# can be combined with ";" (see the RAG service, which uses a burst and a daily
# limit together).
DEFAULT_UPLOAD_RATE_LIMITS = "20/minute"

# Reads are cheap per call, which is exactly why they invite very high call
# rates — /fetch/all is a database scan. Loose enough to never bother a user
# paging through their documents.
DEFAULT_READ_RATE_LIMITS = "120/minute"

# Deletes are cheap too, but they are destructive and no legitimate client
# issues them in bulk.
DEFAULT_DELETE_RATE_LIMITS = "60/minute"

UPLOAD_RATE_LIMITS = resolve_rate_limits("UPLOAD_RATE_LIMITS", DEFAULT_UPLOAD_RATE_LIMITS)
READ_RATE_LIMITS = resolve_rate_limits("READ_RATE_LIMITS", DEFAULT_READ_RATE_LIMITS)
DELETE_RATE_LIMITS = resolve_rate_limits("DELETE_RATE_LIMITS", DEFAULT_DELETE_RATE_LIMITS)

logger.info(
    f"Per-user limits: upload={UPLOAD_RATE_LIMITS}, "
    f"read={READ_RATE_LIMITS}, delete={DELETE_RATE_LIMITS}"
)
